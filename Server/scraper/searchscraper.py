...
def scrape_google(search_term, number_results, language_code):
    ...
    contents = [[x.replace('\n', '').replace('\t', '').replace('\r', '') for x in tex if not x.parent.name in ['style', 'link', 'html', 'ul', 'ol', 'script', 'div', 'table', 'tbody', 'thead', '[document]', 'head', 'title'] and not re.match('<!--.*-->', str(x.encode('utf-8')))] if tex != None else "" for tex in soup_list]
